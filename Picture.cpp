...
Utils Picture::getUtils() {
    return imgio;
}


//Implementation of concurrent_pic
concurrent_pic::concurrent_pic(Picture *picture) {
	this->pic = *picture;
}

concurrent_pic::concurrent_pic() {};

void concurrent_pic::display_pic() {
	pic.getUtils().displayimage(pic.getimage());
}

void concurrent_pic::save_img(string path) {
	pic.getUtils().saveimage(pic.getimage(), path);
}

void concurrent_pic::finish_threads() {
    if(!threads.empty()) {
